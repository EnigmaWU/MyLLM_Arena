...
class TestUs12ManyMergedBranchesPreserveAttributionTdd(unittest.TestCase):
    ...
    def test_cli_resets_one_parallel_rename_lineage_on_main_while_preserving_other_variant(self) -> None:
        query = {
            "vcsType": "git",
            "repoURL": "https://example.local/repo/us12-parallel-main-reset",
            "repoBranch": "main",
            "metric": "live_changed_source_ratio",
            "algorithm": "A",
            "scope": "A",
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            repo = GitRepoHarness(repo_dir)
            repo.write(
                "src/alpha_reset_legacy.py",
                "carry_pre_window = base\n"
                "alpha_human = base + 1\n"
                "alpha_full = base + 2\n",
            )
            repo.write(
                "src/beta_reset_legacy.py",
                "carry_pre_window = base\n"
                "beta_human = base + 1\n"
                "beta_partial = base + 2\n",
            )
            revision_id_r1 = repo.commit_all("us12-pmr-r1", "2026-02-24T09:00:00Z")
            ...
            revision_id_r2 = repo.commit_all("us12-pmr-r2", "2026-03-03T09:00:00Z")
            repo.rename("src/alpha_reset_legacy.py", "src/alpha_reset_final.py")
            revision_id_r3 = repo.commit_all("us12-pmr-r3", "2026-03-04T09:00:00Z")
            repo.write(
                "src/alpha_reset_final.py",
                "carry_pre_window = base\n"
                "alpha_human = sanitize(base + 1)\n"
                "alpha_full = helper(base + 2)\n",
            )
            revision_id_r4 = repo.commit_all("us12-pmr-r4", "2026-03-05T09:00:00Z")
            ...
            revision_id_r5 = repo.commit_all("us12-pmr-r5", "2026-03-06T09:00:00Z")
            repo.rename("src/beta_reset_legacy.py", "src/beta_reset_final.py")
            revision_id_r6 = repo.commit_all("us12-pmr-r6", "2026-03-07T09:00:00Z")
            repo.write(
                "src/beta_reset_final.py",
                "carry_pre_window = base\n"
                "beta_human = normalize(base + 1)\n"
                "beta_partial = suggest(base + 2)\n",
            )
            revision_id_r7 = repo.commit_all("us12-pmr-r7", "2026-03-08T09:00:00Z")

            repo.checkout("main")
            revision_id_r8 = repo.merge_no_ff("feature-alpha", "us12-pmr-r8", "2026-03-10T09:00:00Z")
            revision_id_r9 = repo.merge_no_ff("feature-beta", "us12-pmr-r9", "2026-03-11T09:00:00Z")

            repo.write(
                "src/alpha_reset_final.py",
                "carry_pre_window = base\n"
                "alpha_human = sanitize(base + 1)\n"
                "alpha_full = verify(base + 2)\n",
            )
            revision_id_r10 = repo.commit_all("us12-pmr-r10", "2026-03-12T09:00:00Z")

            repo.write("README.md", "us12 parallel main reset docs update\n")
            revision_id_r11 = repo.commit_all("us12-pmr-r11", "2026-03-13T09:00:00Z")

            baseline_protocol = self._inline_protocol(repo_branch="main", file_name="src/alpha_reset_legacy.py")
            baseline_protocol["DETAIL"] = [
                {"fileName": "src/alpha_reset_legacy.py", "codeLines": []},
                {"fileName": "src/beta_reset_legacy.py", "codeLines": []},
            ]
            write_revision_protocol(protocol_dir, baseline_protocol, repo_dir, revision_id_r1)

            alpha_human_protocol = self._inline_protocol(repo_branch="feature-alpha", file_name="src/alpha_reset_legacy.py")
            alpha_human_protocol["DETAIL"].append({"fileName": "src/beta_reset_legacy.py", "codeLines": []})
            write_revision_protocol(protocol_dir, alpha_human_protocol, repo_dir, revision_id_r2)

            alpha_rename_protocol = self._inline_protocol(repo_branch="feature-alpha", file_name="src/alpha_reset_final.py")
            alpha_rename_protocol["DETAIL"].append({"fileName": "src/beta_reset_legacy.py", "codeLines": []})
            write_revision_protocol(protocol_dir, alpha_rename_protocol, repo_dir, revision_id_r3)

            alpha_ai_protocol = self._inline_protocol(repo_branch="feature-alpha", file_name="src/alpha_reset_final.py", full_lines=[3])
            alpha_ai_protocol["DETAIL"].append({"fileName": "src/beta_reset_legacy.py", "codeLines": []})
            write_revision_protocol(protocol_dir, alpha_ai_protocol, repo_dir, revision_id_r4)

            beta_human_protocol = self._inline_protocol(repo_branch="feature-beta", file_name="src/beta_reset_legacy.py")
            beta_human_protocol["DETAIL"].append({"fileName": "src/alpha_reset_legacy.py", "codeLines": []})
            write_revision_protocol(protocol_dir, beta_human_protocol, repo_dir, revision_id_r5)

            beta_rename_protocol = self._inline_protocol(repo_branch="feature-beta", file_name="src/beta_reset_final.py")
            beta_rename_protocol["DETAIL"].append({"fileName": "src/alpha_reset_legacy.py", "codeLines": []})
            write_revision_protocol(protocol_dir, beta_rename_protocol, repo_dir, revision_id_r6)

            beta_ai_protocol = self._inline_protocol(repo_branch="feature-beta", file_name="src/beta_reset_final.py", partial_lines={3: 60})
            beta_ai_protocol["DETAIL"].append({"fileName": "src/alpha_reset_legacy.py", "codeLines": []})
            write_revision_protocol(protocol_dir, beta_ai_protocol, repo_dir, revision_id_r7)

            alpha_merge_protocol = self._inline_protocol(repo_branch="main", file_name="src/alpha_reset_final.py")
            alpha_merge_protocol["DETAIL"] = [
                {"fileName": "src/alpha_reset_final.py", "codeLines": []},
                {"fileName": "src/beta_reset_legacy.py", "codeLines": []},
            ]
            write_revision_protocol(protocol_dir, alpha_merge_protocol, repo_dir, revision_id_r8)

            beta_merge_protocol = self._inline_protocol(repo_branch="main", file_name="src/beta_reset_final.py")
            beta_merge_protocol["DETAIL"] = [
                {"fileName": "src/alpha_reset_final.py", "codeLines": []},
                {"fileName": "src/beta_reset_final.py", "codeLines": []},
            ]
            write_revision_protocol(protocol_dir, beta_merge_protocol, repo_dir, revision_id_r9)

            main_reset_protocol = self._inline_protocol(repo_branch="main", file_name="src/alpha_reset_final.py")
            main_reset_protocol["DETAIL"] = [
                {"fileName": "src/alpha_reset_final.py", "codeLines": []},
                {"fileName": "src/beta_reset_final.py", "codeLines": []},
            ]
            write_revision_protocol(protocol_dir, main_reset_protocol, repo_dir, revision_id_r10)
            ...
            assert_live_line_log(
                self,
                result.stderr,
                relative_path="src/alpha_reset_final.py",
                final_line=2,
                origin_file="src/alpha_reset_legacy.py",
                origin_line=2,
                revision_id=revision_id_r2,
                classification="human/unattributed",
            )
            assert_live_line_log(
                ...
                relative_path="src/alpha_reset_final.py",
                final_line=3,
                origin_file="src/alpha_reset_final.py",
                origin_line=3,
                revision_id=revision_id_r10,
                classification="human/unattributed",
            )
            assert_live_line_log(
                self,
                result.stderr,
                relative_path="src/beta_reset_final.py",
                final_line=2,
                origin_file="src/beta_reset_legacy.py",
                origin_line=2,
                revision_id=revision_id_r5,
                classification="human/unattributed",
            )
            assert_live_line_log(
                ...
                relative_path="src/beta_reset_final.py",
                final_line=3,
                origin_file="src/beta_reset_final.py",
                origin_line=3,
                revision_id=revision_id_r7,
                classification="60%-ai",
            )
