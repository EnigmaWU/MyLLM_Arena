...
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "testdata" / "us5_rename_preserves_lineage"


class TestUs5RenamePreservesLineageTdd(unittest.TestCase):
    maxDiff = None

    def test_cli_matches_us5_expected_result_for_narrow_algorithm_b_fixture_path(self) -> None:
    ...
    def test_cli_matches_us5_expected_result_when_file_is_renamed_without_rewrite(self) -> None:
        self.assertTrue(
            UTILITY_PATH.exists(),
            f"Expected CLI utility at {UTILITY_PATH} for US-5 end-to-end execution.",
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # WHY: US-5 is specifically about line ownership surviving a pure
            # path rename, so the file content stays unchanged while only the
            # tracked path changes between revisions.
            ...
            revision_id_r1 = repo.commit_all("us5-r1", "2026-03-08T09:00:00Z")

            repo.rename("src/legacy_name.py", "src/current_name.py")
            revision_id_r2 = repo.commit_all("us5-r2", "2026-03-12T09:00:00Z")
    ...
    def test_cli_emits_debug_logs_for_us5_when_enabled(self) -> None:
        query = load_json(FIXTURE_DIR / "query.json")
        revision_protocol_r1 = load_json(FIXTURE_DIR / "01_genCodeDesc.json")
        revision_protocol_r2 = load_json(FIXTURE_DIR / "02_genCodeDesc.json")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert_live_line_log(
                self,
                result.stderr,
                relative_path="src/current_name.py",
                final_line=1,
                origin_file="src/legacy_name.py",
                origin_line=1,
    ...
    def test_cli_info_logging_focuses_on_live_line_aggregation_progress_for_us5(self) -> None:
        query = load_json(FIXTURE_DIR / "query.json")
        revision_protocol_r1 = load_json(FIXTURE_DIR / "01_genCodeDesc.json")
        revision_protocol_r2 = load_json(FIXTURE_DIR / "02_genCodeDesc.json")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            repo.rename("src/legacy_name.py", "src/current_name.py")
            repo.commit_all("us5-r2", "2026-03-12T09:00:00Z")

            write_revision_protocol(protocol_dir, revision_protocol_r1, repo_dir, revision_id_r1)
            write_revision_protocol(protocol_dir, revision_protocol_r2, repo_dir, repo.commit_ids["us5-r2"])
            ...
            assert_log_contains_all(
                ...
                [
                    "Starting analysis",
                    "LiveLine src/current_name.py:1 aggregate",
                    "LiveLine src/current_name.py:2 aggregate",
                    "LiveLine src/current_name.py:3 aggregate",
                ],
            )
