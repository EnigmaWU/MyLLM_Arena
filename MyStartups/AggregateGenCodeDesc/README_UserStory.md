...
Unless a story explicitly defines another metric, `SUMMARY.totalCodeLines` in revision-level `genCodeDesc` always means all non-blank added ('+') code lines in the commit diff. For the current Algorithm A final result, this excludes deleted lines and counts only live lines still present at `endTime`.
