...
If you want inline `genRatio` hints for easy human reading and checking, render a sidecar `commitDiffSetAnnotated/` view with `python3 UserExamplesNG/render_annotated_commit_diff_set.py <datasetDir>`. Keep `commitDiffSet/` itself clean because the aggregate input still comes from the matching `*_genCodeDesc.json` file in `genCodeDescSet/`.
...
If you also want to verify the operator narrative on `stderr`, run the example with `--logLevel info`, redirect `stderr` to a log file, and then run `python3 UserExamplesNG/check_info_log.py ...`. That checker confirms the three phases you care about: begin banner, line-by-line `LiveLine` entries, and final summary.
