...
FIXTURE_GIT_DIR = Path(__file__).resolve().parents[3] / "TestdataNG-AlgB" / "history-complicated" / "scope-a" / "02" / "git" / "default"


class TestUsngAlgbHistoryComplicatedScopeA02Tdd(unittest.TestCase):
    ...
    def test_human_rewrite_resets_ai_attribution(self) -> None:
        expected_result = load_json(FIXTURE_GIT_DIR / "expected_result.json")
        actual_result = self._run_cli(FIXTURE_GIT_DIR)
        self.assertEqual(actual_result, expected_result)
        # Human rewrite: the previously AI-full line is overwritten; fullGeneratedCodeLines must be 0
        self.assertEqual(actual_result["SUMMARY"]["fullGeneratedCodeLines"], 0)
