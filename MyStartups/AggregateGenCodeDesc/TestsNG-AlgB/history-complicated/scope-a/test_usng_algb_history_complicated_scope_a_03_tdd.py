...
FIXTURE_GIT_DIR = Path(__file__).resolve().parents[3] / "TestdataNG-AlgB" / "history-complicated" / "scope-a" / "03" / "git" / "default"


class TestUsngAlgbHistoryComplicatedScopeA03Tdd(unittest.TestCase):
    ...
    def test_ai_rewrite_replaces_human_ownership(self) -> None:
        expected_result = load_json(FIXTURE_GIT_DIR / "expected_result.json")
        actual_result = self._run_cli(FIXTURE_GIT_DIR)
        self.assertEqual(actual_result, expected_result)
        # AI rewrite: the written line is now AI; fullGeneratedCodeLines must be positive
        self.assertGreater(actual_result["SUMMARY"]["fullGeneratedCodeLines"], 0)
