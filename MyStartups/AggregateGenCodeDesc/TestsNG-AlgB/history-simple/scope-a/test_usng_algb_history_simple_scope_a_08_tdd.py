...
FIXTURE_GIT_DIR = Path(__file__).resolve().parents[3] / "TestdataNG-AlgB" / "history-simple" / "scope-a" / "08" / "git" / "default"
FIXTURE_SVN_DIR = Path(__file__).resolve().parents[3] / "TestdataNG-AlgB" / "history-simple" / "scope-a" / "08" / "svn" / "default"


class TestUsngAlgbHistorySimpleScopeA08Tdd(unittest.TestCase):
    ...
    def test_git_and_svn_summary_are_identical(self) -> None:
        git_result = self._run_cli(FIXTURE_GIT_DIR)
        svn_result = self._run_cli(FIXTURE_SVN_DIR)
        self.assertEqual(git_result["SUMMARY"], svn_result["SUMMARY"])
