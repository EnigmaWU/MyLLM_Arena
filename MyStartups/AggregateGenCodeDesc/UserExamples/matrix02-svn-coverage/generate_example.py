...
def write_helper_scripts(project_root: Path, output_dir: Path, repo_url: str) -> None:
    script_path = output_dir / "run_example.sh"
    check_path = output_dir / "check_output.sh"
    ...
    script_path.write_text(
        ...
        'OUTPUT_FILE="${3:-/tmp/agg-userexample-matrix02-svn-out.json}"\n'
        f'REPO_ROOT={json.dumps(str(project_root))}\n'
        f'EXAMPLE_DIR={json.dumps(str(output_dir))}\n'
        ...
        '    ;;\n'
        '  algb-svn-workflow|algb-offline)\n'
        '    python3 "$REPO_ROOT/aggregateGenCodeDesc.py" \\\n'
        '      --vcsType svn \\\n'
        '      --repoURL "$REPO_URL" \\\n'
        '      --repoBranch trunk \\\n'
        ...
        '      --outputFile "$OUTPUT_FILE" \\\n'
        '      --genCodeDescSetDir "$EXAMPLE_DIR/genCodeDescSet" \\\n'
        '      --commitDiffSetDir "$EXAMPLE_DIR/commitDiffSet"\n'
        '    ;;\n'
        '  algc)\n'
        '    python3 "$REPO_ROOT/aggregateGenCodeDesc.py" \\\n'
        '      --algorithm C \\\n'
        ...
        '      --scope "$SCOPE" \\\n'
        '      --outputFile "$OUTPUT_FILE" \\\n'
        '      --genCodeDescSetDir "$EXAMPLE_DIR/algcGenCodeDescSet"\n'
        '    ;;\n'
        '  *)\n'
    ...
    check_path.write_text(
        ...
        'OUTPUT_FILE="${3:-/tmp/agg-userexample-matrix02-svn-out.json}"\n'
        f'EXAMPLE_DIR={json.dumps(str(output_dir))}\n\n'
        'python3 - "$MODE" "$SCOPE" "$OUTPUT_FILE" "$EXAMPLE_DIR" <<\'PY\'\n'
        'import json\n'
        'import sys\n'
        'from pathlib import Path\n\n'
        'mode = sys.argv[1]\n'
        ...
        'expected = json.loads((Path(sys.argv[4]) / f"expected_{mode}_{scope}.json").read_text())\n\n'
        'if actual != expected:\n'
        '    raise SystemExit(f"Matrix SVN example mismatch for mode {mode} scope {scope}")\n\n'
        'print(f"Matrix SVN example OK for mode {mode} scope {scope}")\n'
        'PY\n',
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    write_helper_scripts(Path(__file__).resolve().parents[2], output_dir, repo_url)
