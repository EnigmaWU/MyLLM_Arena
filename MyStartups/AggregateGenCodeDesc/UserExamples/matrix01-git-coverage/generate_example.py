...
def write_helper_scripts(project_root: Path, output_dir: Path) -> None:
    ...
    script_path.write_text(
        ...
        'OUTPUT_FILE="${3:-/tmp/agg-userexample-matrix01-git-out.json}"\n'
    ...
    check_path.write_text(
        ...
        'if actual != expected:\n'
        '    raise SystemExit(f"Matrix Git example mismatch for mode {mode} scope {scope}")\n\n'
        'print(f"Matrix Git example OK for mode {mode} scope {scope}")\n'
        'PY\n',
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    write_helper_scripts(Path(__file__).resolve().parents[2], output_dir)
