...
- `us2_human_overwrites_ai_live_changed` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): later human rewrite resets one live changed line to `0`, expected `66.67%`
- `us3_ai_overwrites_human_live_changed` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): later AI rewrite takes ownership of one live changed line, expected `60.0%`
- `us4_deleted_lines_excluded` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): deleted lines are excluded from the live changed snapshot, expected `100%`
- `us5_rename_preserves_lineage` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): rename preserves original attribution lineage, expected `66.67%`
...
- `us7_mixed_multi_commit_window` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): mixed multi-commit history with human-only lines, AI-only lines, human-then-AI rewrites, AI-then-human rewrites, and deleted AI lines, expected final summary `total=5, full=1, partial=1`
- `us8_merge_commit_preserves_attribution` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): merge commit keeps effective attribution of surviving lines, expected final summary `total=4, full=1, partial=0`
...
- `us10_large_repository_snapshot` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): large-snapshot semantics remain stable under broad file and line counts
- `us11_deep_history_preserves_attribution` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): deep history preserves latest effective attribution without leaking superseded states
- `us12_many_merged_branches_preserve_attribution` (`Shared US`, current active evidence is `Algorithm A` plus narrow Git `Algorithm B`): branch-heavy merges preserve effective per-line attribution
...
- Current convergence interpretation in `testdata`:
  ...
  - `US-2`, `US-3`, and `US-4` now have current active evidence on `Algorithm A` plus narrow Git `Algorithm B` live-snapshot replay for the approved baseline fixture shape
  - `US-5` and `US-7` now have current active evidence on `Algorithm A` plus narrow Git `Algorithm B` live-snapshot replay for the approved baseline fixture shape
  - `US-8` and `US-12` now have current active evidence on `Algorithm A` plus narrow Git `Algorithm B` live-snapshot replay for the approved baseline fixture shape
  - `US-10` and `US-11` now have current active evidence on `Algorithm A` plus narrow Git `Algorithm B` live-snapshot replay for the approved baseline fixture shape
  - `US-9` is a shared contract story whose current active evidence is Git/SVN parity through `Algorithm A` plus narrow Git/SVN `Algorithm B` parity on the approved `US-1` baseline shape
